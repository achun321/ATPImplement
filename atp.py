"""
Copied from VideoBERT: https://github.com/ammesatyajit/VideoBERT. 
...
class VideoTransformer(nn.Module):
    def __init__(self, config, args):
        ...
        self.scale = torch.sqrt(torch.FloatTensor([self.config.hidden_size])).to(
            self.args.device
        )
    ...
    def forward(
        ...
        if text_input_ids is not None:
            text_mask = self._generate_square_subsequent_mask(
                text_input_ids.shape[1] - 1
            ).to(self.args.device)
            ...
            text_loss = loss_fct(
                text_out,
                text_input_ids[:, 1:].contiguous().view(-1).to(self.args.device),
            )
            outputs.append(text_loss)

        if video_input_ids is not None:
            vid_mask = self._generate_square_subsequent_mask(
                video_input_ids.shape[1] - 1
            ).to(self.args.device)
            ...
            video_loss = loss_fct(
                vid_out,
                video_input_ids[:, 1:].contiguous().view(-1).to(self.args.device),
            )
            outputs.append(video_loss)

        if joint_input_ids is not None:
            joint_mask = self._generate_square_subsequent_mask(
                joint_input_ids.shape[1] - 1
            ).to(self.args.device)
            ...
            joint_loss = loss_fct(
                joint_out,
                joint_input_ids[:, 1:].contiguous().view(-1).to(self.args.device),
            )
    ...
    def from_pretrained(cls, config, args):
        ...
        model.load_state_dict(
            torch.load(args.model_name_or_path + "/pytorch_model.bin")
        )
        model.eval()
        return model
